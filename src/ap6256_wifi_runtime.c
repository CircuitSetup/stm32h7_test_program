...
#define AP6256_WIFI_JOIN_F2_BLOCK_SIZE     AP6256_WIFI_SCAN_RECOVERY_F2_BLOCK_SIZE
#define AP6256_WIFI_DIRECTED_5G_SCAN_TIMEOUT_MS 15000U
#define AP6256_WIFI_PROFILE_BROAD_SCAN_TIMEOUT_MS 8000U
#define AP6256_WIFI_5G_RESET_PRECURSOR_MS 2000U
#define AP6256_WIFI_5G_ASSOC_QUIET_MS 0U
#define AP6256_WIFI_5G_ASSOC_PROBE_DELAY_MS 1000U
#define AP6256_WIFI_5G_KEY_WAIT_MS 3000U
/*
 * Keep 5 GHz joins bounded without treating unrelated high-numbered Broadcom
 * events as association progress. Event 124 is CCA channel-quality telemetry in
 * Broadcom DHD headers, so it must not extend the join window or start DHCP.
 */
#ifndef AP6256_WIFI_5G_JOIN_NO_PROGRESS_MS
#define AP6256_WIFI_5G_JOIN_NO_PROGRESS_MS 2000U
#endif
#ifndef AP6256_WIFI_5G_ASSOC_PROGRESS_WAIT_MS
#define AP6256_WIFI_5G_ASSOC_PROGRESS_WAIT_MS 8000U
#endif
...
#define AP6256_CYW43_JOIN_STATE_ACTIVE    0x0001U
...
#ifndef AP6256_WIFI_5G_USE_TRANSITION_AUTH
#define AP6256_WIFI_5G_USE_TRANSITION_AUTH 0
#endif

#ifndef AP6256_WIFI_5G_FORCE_SAE_AUTH
#define AP6256_WIFI_5G_FORCE_SAE_AUTH 0
#endif

#ifndef AP6256_WIFI_5G_PRIMARY_STARTER_FIRST
#define AP6256_WIFI_5G_PRIMARY_STARTER_FIRST 0
#endif
#ifndef AP6256_WIFI_5G_PRIMARY_STARTER_RETRY
#define AP6256_WIFI_5G_PRIMARY_STARTER_RETRY 0
#endif
#ifndef AP6256_WIFI_5G_RECYCLE_AFTER_SCAN
#define AP6256_WIFI_5G_RECYCLE_AFTER_SCAN 0
#endif
#ifndef AP6256_WIFI_5G_FULL_RECYCLE_AFTER_FAILED_CANDIDATE
#define AP6256_WIFI_5G_FULL_RECYCLE_AFTER_FAILED_CANDIDATE 0
#endif
#ifndef AP6256_WIFI_5G_STOP_AFTER_POISONED_SESSION
#define AP6256_WIFI_5G_STOP_AFTER_POISONED_SESSION 1
#endif
#ifndef AP6256_WIFI_5G_DIRECTED_JOIN
#define AP6256_WIFI_5G_DIRECTED_JOIN 0
#endif

#ifndef AP6256_WIFI_5G_BSSID_HINT
#define AP6256_WIFI_5G_BSSID_HINT 0
#endif
...
static void ap6256_wifi_runtime_capture_assoc_diag(const char *failure_class,
                                                   int32_t link_status,
...
static bool ap6256_wifi_runtime_prepare_join_sdio_policy(char *detail, size_t detail_len)
{
    ...
    /*
     * On this STM32H7/AP6256 path the BCM43456 firmware can reset when the
     * association command is sent immediately after scan with a 512-byte F2
     * policy. Keep the proven scan-mode F2 framing through WLC_SET_SSID; the
     * data path can be promoted later after link evidence is stable.
    */
    ...
    if (block_rc != 0) {
        if ((detail != NULL) && (detail_len != 0U)) {
            (void)snprintf(detail,
                           detail_len,
                           "Failed to apply join F2 block size %u rc=%ld.",
                                                            ...
                                                            uint32_t *unsupported_count)
{
    ...
    /*
     * Keep candidate selection evidence-based: brcmfmac walks compatible BSS
     * entries from the scan table, and the strongest RSSI candidate is the
     * least surprising first association target. Earlier lower-channel bucket
     * steering made the all-channel doorbelkin run pick ch44 before the
     * stronger ch149 BSS, then poisoned the firmware session before the best
    ...
    for (uint32_t i = 0U; i < candidate_count; ++i) {
        for (uint32_t j = i + 1U; j < candidate_count; ++j) {
            ...
            if (candidates[j].rssi > candidates[i].rssi) {
                swap = 1U;
            } else if ((candidates[j].rssi == candidates[i].rssi) &&
                       (candidates[j].channel < candidates[i].channel)) {
                swap = 1U;
            }
...
directed_scan_retry:
    ...
    if (rc != 0) {
        ...
        if ((scan_start_recovery_attempted == 0U) &&
            ...
            test_uart_write_str("[ INFO ] wifi.connect stage: directed 5GHz scan start recovery\r\n");
            if (((ap6256_cyw43_port_last_ioctl_phase() == AP6256_CYW43_IOCTL_PHASE_WAIT_NO_PACKET) ||
                ...
                test_uart_printf("[ INFO ] wifi.connect stage: directed retry with F2 block size 64 after %s at bs=%lu\r\n",
            ...
            if (block_rc != 0) {
                (void)snprintf(detail,
                               detail_len,
                               "Failed to apply directed 5GHz scan-recovery F2 block size %lu rc=%ld.",
                               (unsigned long)ap6256_cyw43_port_runtime_f2_block_size(),
                               (long)block_rc);
                return NULL;
            }
            goto directed_scan_retry;
        }
        (void)snprintf(detail,
                       detail_len,
                       "Directed 5GHz scan for SSID '%s' failed to start (rc=%d ph=%s).",
                       ssid,
                       rc,
                       ap6256_wifi_runtime_ioctl_phase_name(ap6256_cyw43_port_last_ioctl_phase()));
        return NULL;
...
static uint8_t ap6256_wifi_runtime_last_event_is_keyed(void)
{
    return ((ap6256_cyw43_port_last_join_event_type() == CYW43_EV_PSK_SUP) &&
            (ap6256_cyw43_port_last_join_event_status() == CYW43_SUP_KEYED)) ? 1U : 0U;
}

static uint8_t ap6256_wifi_runtime_last_event_is_assoc_progress(void)
{
    switch (ap6256_cyw43_port_last_join_event_type()) {
    case CYW43_EV_PRE_ASSOC_IND:
    case CYW43_EV_PRE_REASSOC_IND:
        /*
         * brcmfmac names event 61/62 as PRE_ASSOC/PRE_REASSOC indication.
         * This is useful connect-attempt evidence, but not auth/link/key proof.
         */
        return (ap6256_cyw43_port_last_join_event_status() == CYW43_STATUS_SUCCESS) ? 1U : 0U;
    default:
        return 0U;
    }
}

static uint8_t ap6256_wifi_runtime_join_event_supports_assoc_probe(void)
{
    switch (ap6256_cyw43_port_last_join_event_type()) {
    case CYW43_EV_PRE_ASSOC_IND:
    case CYW43_EV_PRE_REASSOC_IND:
...
static uint8_t ap6256_wifi_runtime_should_probe_assoc_while_down(uint8_t selected_5g,
                                                                 int status,
                                                                 uint8_t post_join_event_seen)
{
    if (selected_5g == 0U) {
    ...
    if (post_join_event_seen == 0U) {
        return 0U;
    }
    return ap6256_wifi_runtime_join_event_supports_assoc_probe();
}

static uint8_t ap6256_wifi_runtime_5g_can_probe_bssid(uint8_t secure,
                                                      uint8_t assoc_probe_event,
                                                      uint8_t keyed_seen)
{
    if (secure == 0U) {
        return (assoc_probe_event != 0U) ? 1U : 0U;
    }
    if (keyed_seen != 0U) {
        return 1U;
    }
    if (ap6256_wifi_runtime_last_event_is_keyed() != 0U) {
        return 1U;
    }
    return 0U;
}

static uint8_t ap6256_wifi_runtime_should_retry_primary_channel_starter(const ap6256_wifi_scan_entry_t *entry,
                                                                        ap6256_status_t status,
                                                                        uint8_t candidate_index,
                                                                        const char *detail)
{
#if AP6256_WIFI_5G_PRIMARY_STARTER_FIRST != 0
    /*
     * The normal 5 GHz path already used the conservative primary-channel
     * starter. Do not run the same association command shape twice inside one
     * candidate/session; that only extends a known dead JOIN state.
     */
    (void)entry;
    (void)status;
    ...
    (void)detail;
    return 0U;
#elif AP6256_WIFI_5G_PRIMARY_STARTER_RETRY == 0
    /*
     * Current HIL logs show no-starter and primary-starter joins both reach the
     * same SET_SSID/no-AUTH state on doorbelkin. Keep the retry disabled by
     * default so one test run does not internally repeat a known-bad shape.
     */
    (void)entry;
    (void)status;
    (void)candidate_index;
    ...
    if (ap6256_wifi_runtime_channel_is_5g(entry->channel) == 0U) {
        return 0U;
    }
    if ((entry->secure == 0U) || (entry->channel == 0U) ||
        (entry->channel == AP6256_WIFI_CHANNEL_5G_UNKNOWN)) {
        return 0U;
    }
    if ((status == AP6256_STATUS_OK) || (status == AP6256_STATUS_BAD_PARAM)) {
        return 0U;
    }
    if (strstr(detail, "Wi-Fi join request failed") != NULL) {
        return 0U;
    }
    if (strstr(detail, "SSID disappeared or was not found") != NULL) {
        /*
         * 5 GHz best-candidate runs can stall in a no-progress timeout with an
         * opaque last event (for example ev=2/0/0) even though the bounded
         * primary-channel starter retry is the next safest recovery step.
         * Keep this fallback available for the first candidate without
         * requiring one of the narrower auth/link event codes.
         */
        return 1U;
    }
    if ((strstr(detail, "WPA/WPA2 key exchange did not complete") == NULL) &&
        (strstr(detail, "authentication failed") == NULL)) {
        return 0U;
    }
    switch (ap6256_cyw43_port_last_join_event_type()) {
    case CYW43_EV_AUTH:
...
static void ap6256_wifi_runtime_force_keyed_link(void)
{
    cyw43_state.wifi_join_state =
        AP6256_CYW43_JOIN_STATE_ACTIVE |
        AP6256_CYW43_JOIN_STATE_AUTH |
        AP6256_CYW43_JOIN_STATE_LINK |
        AP6256_CYW43_JOIN_STATE_KEYED;
...
static bool ap6256_wifi_runtime_wait_for_link(uint32_t timeout_ms,
                                              int *final_status,
                                              const uint8_t *scan_bssid_hint,
                                              const uint8_t *target_bssid_diag,
                                              uint16_t target_channel,
                                              uint16_t target_chanspec,
                                              uint32_t auth_type,
                                              uint8_t secure,
                                              uint8_t selected_5g)
{
    ...
    uint32_t first_assoc_seen_ms = 0U;
    uint32_t start_event_count = ap6256_cyw43_port_join_event_count();
    uint8_t bcm43456_keyed_seen = 0U;
    ...
    if (selected_5g != 0U) {
        /*
         * Do not sit in a blind post-WLC_SET_SSID delay on 5 GHz. HIL has now
         * shown both the old multi-second wait and a 750 ms "short" wait can
         * reset before the RX loop drains stale scan-tail events. Fall through
         * immediately so join progress is classified from live firmware events.
         */
        ap6256_cyw43_port_record_breadcrumb(AP6256_CYW43_BREADCRUMB_JOIN_WAIT, -300);
        if (AP6256_WIFI_5G_ASSOC_QUIET_MS != 0U) {
            osDelay(AP6256_WIFI_5G_ASSOC_QUIET_MS);
        }
    }
    ...
    while ((HAL_GetTick() - start_ms) < timeout_ms) {
        ...
        if ((secure != 0U) && (ap6256_wifi_runtime_last_event_is_keyed() != 0U)) {
            bcm43456_keyed_seen = 1U;
            if ((cyw43_state.wifi_join_state & AP6256_CYW43_JOIN_STATE_KEYED) == 0U) {
                ap6256_wifi_runtime_force_keyed_link();
         ...
         * CYW43 43439 state machine remains at LINK_JOIN. On 5 GHz secure
         * joins, exact-BSSID association proof is useful for diagnostics but
         * not sufficient to start DHCP; auth failures still win via
         * PSK/AUTH/DEAUTH events below.
         */
        uint8_t post_join_event_seen =
        ...
        uint8_t assoc_probe_event =
            (ap6256_wifi_runtime_join_event_supports_assoc_probe() != 0U) ? 1U : 0U;
        uint8_t assoc_progress_active = 0U;
        uint32_t reset_precursor_timeout_ms = AP6256_WIFI_5G_RESET_PRECURSOR_MS;
        uint32_t no_progress_timeout_ms = AP6256_WIFI_5G_JOIN_NO_PROGRESS_MS;

        if ((selected_5g != 0U) &&
            (post_join_link_evidence == 0U) &&
            (last_assoc_seen == 0U) &&
            (ap6256_wifi_runtime_last_event_is_assoc_progress() != 0U)) {
            assoc_progress_active = 1U;
            /*
             * Only true association evidence may widen this window. Event 124
             * is Broadcom CCA channel-quality telemetry, not join progress.
             */
            reset_precursor_timeout_ms = AP6256_WIFI_5G_ASSOC_PROGRESS_WAIT_MS;
            no_progress_timeout_ms = AP6256_WIFI_5G_ASSOC_PROGRESS_WAIT_MS;
        }

        if ((selected_5g != 0U) &&
            (dhcp_restart_done == 0U) &&
            (status >= CYW43_LINK_NOIP) &&
            (ap6256_wifi_runtime_5g_can_probe_bssid(secure,
                                                    assoc_probe_event,
                                                    bcm43456_keyed_seen) != 0U) &&
            ((now_ms - last_assoc_probe_ms) >= 250U)) {
            uint8_t assoc_bssid[6];
            int assoc_seen;

            /*
             * BCM43456 firmware/NVRAM profiles can raise the CYW43 netif link
             * from association-progress events before the selected BSSID is
             * actually installed. brcmfmac does not start the data path from a
             * paper link; it waits for concrete association evidence. exact-BSSID
             * association proof is useful for diagnostics but not sufficient to
             * start DHCP. Keep DHCP idle until GET_BSSID proves the selected
             * 5 GHz candidate and a real keyed event arrives.
             */
            ...
            if (last_assoc_seen != 0U) {
                ...
                if (first_assoc_seen_ms == 0U) {
                    first_assoc_seen_ms = now_ms;
                }
                last_assoc_matches =
                    ((scan_bssid_hint == NULL) ||
                     (ap6256_wifi_runtime_bssid_matches(assoc_bssid, scan_bssid_hint) != 0)) ? 1U : 0U;
            } else {
            ...
            if ((last_assoc_matches == 0U) ||
                ...
                test_uart_printf("[ INFO ] wifi.connect stage: hold DHCP until 5GHz BSSID proof status=%d join=0x%08lX assoc=%u/m%u %02X:%02X:%02X:%02X:%02X:%02X\r\n",
                                 status,
                                 ...
                                 last_assoc_bssid[4],
                                 last_assoc_bssid[5]);
                cyw43_cb_tcpip_set_link_down(&cyw43_state, CYW43_ITF_STA);
                (void)netifapi_dhcp_release_and_stop(sta_netif);
        ...
        if (((status == CYW43_LINK_JOIN) ||
             (ap6256_wifi_runtime_should_probe_assoc_while_down(selected_5g,
                                                                status,
            ...
            (((selected_5g == 0U) &&
             ...
             ((selected_5g != 0U) &&
              ((now_ms - start_ms) >= AP6256_WIFI_5G_ASSOC_PROBE_DELAY_MS))) &&
            ((now_ms - last_assoc_probe_ms) >= 250U)) {
            uint8_t assoc_bssid[6];
            int assoc_seen;
            ...
            if ((last_assoc_matches != 0U) &&
                ...
                test_uart_printf("[ INFO ] wifi.connect stage: association confirmed; start DHCP bssid=%02X:%02X:%02X:%02X:%02X:%02X join=0x%08lX rx=%s/%u\r\n",
                                 assoc_bssid[0],
                                 assoc_bssid[1],
                                 assoc_bssid[2],
                                 assoc_bssid[3],
        ...
        if ((selected_5g != 0U) &&
            ...
            (last_assoc_seen != 0U) &&
            (last_assoc_matches != 0U) &&
            ((bcm43456_keyed_seen != 0U) || (ap6256_wifi_runtime_last_event_is_keyed() != 0U))) {
            struct netif *sta_netif = &cyw43_state.netif[CYW43_ITF_STA];

            bcm43456_keyed_seen = 1U;
            ap6256_wifi_runtime_force_keyed_link();
            dhcp_restart_done = 1U;
            test_uart_printf("[ INFO ] wifi.connect stage: 5GHz key evidence accepted; start DHCP bssid=%02X:%02X:%02X:%02X:%02X:%02X ev=%lu/%lu/%lu\r\n",
                             last_assoc_bssid[0],
                             last_assoc_bssid[1],
                             last_assoc_bssid[2],
                             last_assoc_bssid[3],
                             last_assoc_bssid[4],
                             last_assoc_bssid[5],
                             (unsigned long)ap6256_cyw43_port_last_join_event_type(),
                             (unsigned long)ap6256_cyw43_port_last_join_event_status(),
                             (unsigned long)ap6256_cyw43_port_last_join_event_reason());
            ap6256_wifi_runtime_restart_dhcp_after_link(sta_netif);
            ap6256_wifi_runtime_poll_burst(24U, 4U);
            status = CYW43_LINK_NOIP;
            if (final_status != NULL) {
        ...
        if ((selected_5g != 0U) &&
            ...
            (last_assoc_seen != 0U) &&
            (last_assoc_matches != 0U) &&
            (first_assoc_seen_ms != 0U) &&
            ((cyw43_state.wifi_join_state & AP6256_CYW43_JOIN_STATE_KEYED) == 0U) &&
            (bcm43456_keyed_seen == 0U) &&
            (ap6256_wifi_runtime_last_event_is_keyed() == 0U) &&
            ((now_ms - first_assoc_seen_ms) >= AP6256_WIFI_5G_KEY_WAIT_MS)) {
            if (final_status != NULL) {
                *final_status = CYW43_LINK_BADAUTH;
            }
            test_uart_printf("[ INFO ] wifi.connect stage: 5GHz assoc reached BSSID but no key %lums/%lums join=0x%08lX assoc=%u/m%u %02X:%02X:%02X:%02X:%02X:%02X ev=%lu/%lu/%lu rx=%s/%u; auth failure\r\n",
                             (unsigned long)(now_ms - first_assoc_seen_ms),
                             (unsigned long)AP6256_WIFI_5G_KEY_WAIT_MS,
                             (unsigned long)cyw43_state.wifi_join_state,
                             last_assoc_seen,
                             last_assoc_matches,
                             last_assoc_bssid[0],
                             ...
                             ap6256_wifi_runtime_rx_class_name(ap6256_cyw43_port_last_rx_class()),
                             ap6256_cyw43_port_last_rx_payload_len());
            ap6256_wifi_runtime_capture_assoc_diag("LINK_NO_4WAY",
                                                   CYW43_LINK_BADAUTH,
                                                   target_bssid_diag,
                                                   target_channel,
                                                   target_chanspec,
                                                   ...
                                                   last_assoc_matches,
                                                   last_assoc_bssid,
                                                   post_join_event_seen,
                                                   post_join_link_evidence,
                                                   bcm43456_keyed_seen);
            ap6256_cyw43_port_record_breadcrumb(AP6256_CYW43_BREADCRUMB_JOIN_RESULT,
                                                CYW43_LINK_BADAUTH);
            return false;
        }
        ...
        if ((status == CYW43_LINK_BADAUTH) &&
            ...
            /*
             * Some CYW43 state paths can still leave the kind bits at BADAUTH
             * from an earlier transient. brcmfmac completes connection when
             * association success and real firmware-supplicant completion are
             * both observed, so let that key-complete event repair stale kind
             * bits and keep DHCP alive.
             */
            bcm43456_keyed_seen = 1U;
            ap6256_wifi_runtime_force_keyed_link();
            if (dhcp_restart_done == 0U) {
                dhcp_restart_done = 1U;
                ap6256_wifi_runtime_restart_dhcp_after_link(sta_netif);
            }
            status = CYW43_LINK_NOIP;
            ...
            test_uart_printf("[ INFO ] wifi.connect stage: 5GHz key-complete recovered stale badauth join=0x%08lX ev=%lu/%lu/%lu\r\n",
                             (unsigned long)cyw43_state.wifi_join_state,
                             (unsigned long)ap6256_cyw43_port_last_join_event_type(),
                             (unsigned long)ap6256_cyw43_port_last_join_event_status(),
                             (unsigned long)ap6256_cyw43_port_last_join_event_reason());
        ...
        if ((status == CYW43_LINK_FAIL) ||
            ...
            if ((selected_5g != 0U) && (status == CYW43_LINK_BADAUTH)) {
                /*
                 * Drain one tiny grace window before treating BADAUTH as
                 * terminal so a late real PSK_SUP event can still be consumed.
                 * Event 76 is WAKE_EVENT telemetry and is not key evidence.
                 */
                ap6256_wifi_runtime_poll_burst(8U, 2U);
            }
            if ((status == CYW43_LINK_BADAUTH) &&
                ((bcm43456_keyed_seen != 0U) || (ap6256_wifi_runtime_last_event_is_keyed() != 0U))) {
                struct netif *sta_netif = &cyw43_state.netif[CYW43_ITF_STA];
                ...
                test_uart_printf("[ INFO ] wifi.connect stage: keyed event overrides stale badauth; start DHCP ev=%lu/%lu/%lu\r\n",
                                 (unsigned long)ap6256_cyw43_port_last_join_event_type(),
                                 (unsigned long)ap6256_cyw43_port_last_join_event_status(),
                                 (unsigned long)ap6256_cyw43_port_last_join_event_reason());
                continue;
            ...
            if ((selected_5g != 0U) &&
                ...
                (last_assoc_seen != 0U) &&
                (last_assoc_matches != 0U) &&
                (ap6256_cyw43_port_last_join_event_type() != CYW43_EV_DEAUTH) &&
                (ap6256_cyw43_port_last_join_event_type() != CYW43_EV_DEAUTH_IND) &&
                (ap6256_cyw43_port_last_join_event_type() != CYW43_EV_DISASSOC) &&
                (ap6256_cyw43_port_last_join_event_type() != CYW43_EV_DISASSOC_IND)) {
                struct netif *sta_netif = &cyw43_state.netif[CYW43_ITF_STA];
                ...
                test_uart_printf("[ INFO ] wifi.connect stage: 5GHz assoc evidence recovered terminal state join=0x%08lX ev=%lu/%lu/%lu bssid=%02X:%02X:%02X:%02X:%02X:%02X\r\n",
                                 (unsigned long)cyw43_state.wifi_join_state,
                                 (unsigned long)ap6256_cyw43_port_last_join_event_type(),
                                 (unsigned long)ap6256_cyw43_port_last_join_event_status(),
                                 (unsigned long)ap6256_cyw43_port_last_join_event_reason(),
            ...
            const char *failure_class =
                (selected_5g == 0U) ? "JOIN_FAILED" :
                ((status == CYW43_LINK_BADAUTH) ?
                    ((last_assoc_seen != 0U) ? "LINK_NO_4WAY" : "NO_AUTH_AP_REJECT_OR_IGNORE") :
                    "NO_AUTH_TX_UNKNOWN");

            ap6256_wifi_runtime_capture_assoc_diag(failure_class,
                                                   status,
                                                   target_bssid_diag,
                                                   target_channel,
                                                   target_chanspec,
                                                   ...
                                                   post_join_link_evidence,
                                                   bcm43456_keyed_seen);
            test_uart_printf("[ INFO ] wifi.connect stage: join terminal failure_class=%s status=%d elapsed=%lums join=0x%08lX assoc=%u/m%u %02X:%02X:%02X:%02X:%02X:%02X ev=%lu/%lu/%lu r=%lu f=0x%lX rx=%s/%u/%04X/%u/%u>%u/%08lX tx=%u/%u/%04X/%u/%u>%u/%ld sm=%08lX/%04X ch=%08lX/%04X fc=%u/%u/%u/%u/%ld\r\n",
                             failure_class,
                             status,
                             ...
                             (unsigned long)ap6256_cyw43_port_last_join_event_status(),
                             (unsigned long)ap6256_cyw43_port_last_join_event_reason(),
                             (unsigned long)ap6256_cyw43_port_last_join_event_flags(),
                             ap6256_wifi_runtime_rx_class_name(ap6256_cyw43_port_last_rx_class()),
                             ap6256_cyw43_port_last_rx_payload_len(),
        ...
        if ((selected_5g != 0U) &&
            ...
            (status == CYW43_LINK_JOIN) &&
            (post_join_link_evidence == 0U) &&
            ((now_ms - start_ms) >= no_progress_timeout_ms)) {
            if (final_status != NULL) {
                *final_status = CYW43_LINK_NONET;
            }
            ap6256_wifi_runtime_capture_assoc_diag("NO_AUTH_TX_UNKNOWN",
                                                   CYW43_LINK_NONET,
                                                   target_bssid_diag,
                                                   target_channel,
                                                   target_chanspec,
                                                   ...
                                                   post_join_link_evidence,
                                                   bcm43456_keyed_seen);
            test_uart_printf("[ INFO ] wifi.connect stage: 5GHz join stuck in JOIN %lums/%lums failure_class=NO_AUTH_TX_UNKNOWN join=0x%08lX assoc=%u/m%u ev=%lu/%lu/%lu rx=%s/%u; try next candidate\r\n",
                             (unsigned long)(now_ms - start_ms),
                             (unsigned long)no_progress_timeout_ms,
                             (unsigned long)cyw43_state.wifi_join_state,
                             last_assoc_seen,
                             last_assoc_matches,
                             (unsigned long)ap6256_cyw43_port_join_event_count(),
        ...
        if ((selected_5g != 0U) &&
            (post_join_link_evidence == 0U) &&
            (last_assoc_seen == 0U) &&
            ((now_ms - start_ms) >= reset_precursor_timeout_ms) &&
            (ap6256_cyw43_port_join_event_count() != 0U) &&
            (ap6256_cyw43_port_last_join_event_type() == CYW43_EV_SET_SSID)) {
            if (final_status != NULL) {
                *final_status = CYW43_LINK_NONET;
            }
            ap6256_wifi_runtime_capture_assoc_diag("NO_AUTH_TX_UNKNOWN",
                                                   ...
                                                   post_join_link_evidence,
                                                   bcm43456_keyed_seen);
            test_uart_printf("[ INFO ] wifi.connect stage: 5GHz join reset-precursor %lums/%lums failure_class=NO_AUTH_TX_UNKNOWN status=%d join=0x%08lX ev=%lu/%lu/%lu f=0x%lX ap=%u rx=%s/%u; try next candidate\r\n",
                             (unsigned long)(now_ms - start_ms),
                             (unsigned long)reset_precursor_timeout_ms,
                             status,
                             (unsigned long)cyw43_state.wifi_join_state,
                             (unsigned long)ap6256_cyw43_port_join_event_count(),
                             (unsigned long)ap6256_cyw43_port_last_join_event_type(),
        ...
        if ((selected_5g != 0U) &&
            ...
            ap6256_wifi_runtime_capture_assoc_diag("NO_AUTH_TX_UNKNOWN",
                                                   ...
                                                   post_join_link_evidence,
                                                   bcm43456_keyed_seen);
            test_uart_printf("[ INFO ] wifi.connect stage: 5GHz join no progress %lums/%lums failure_class=NO_AUTH_TX_UNKNOWN status=%d join=0x%08lX ev=%lu/%lu/%lu rx=%s/%u; try next candidate\r\n",
                             (unsigned long)(now_ms - start_ms),
                             (unsigned long)no_progress_timeout_ms,
                             status,
                             (unsigned long)cyw43_state.wifi_join_state,
                                                ...
                                                const char *password)
{
    ...
    s_wifi_runtime.cached_rsn_cap = entry->rsn_cap;
...
static bool ap6256_wifi_runtime_recycle_after_5g_scan_before_join(uint32_t candidate_index,
                                                                  uint32_t candidate_count,
                                                                  char *detail,
                                                                  size_t detail_len)
{
#if AP6256_WIFI_5G_RECYCLE_AFTER_SCAN != 0
    test_uart_printf("[ INFO ] wifi.connect stage: full recycle after 5GHz scan before join candidate %lu/%lu\r\n",
    ...
    if (s_wifi_runtime.initialized != 0U) {
        ...
        s_wifi_runtime.stack_ready = 0U;
        s_wifi_runtime.link_up = 0U;
        s_wifi_runtime.poisoned_5g_session = 0U;
        osDelay(120U);
    }
    return ap6256_wifi_runtime_ensure_ready(detail, detail_len);
#else
    test_uart_printf("[ INFO ] wifi.connect stage: bounded drain after 5GHz scan before join candidate %lu/%lu\r\n",
                     (unsigned long)candidate_index,
                     (unsigned long)candidate_count);
    return ap6256_wifi_runtime_recover_radio_before_join("after 5GHz scan",
                                                         detail,
                                                         detail_len);
#endif
}

static bool ap6256_wifi_runtime_recycle_after_failed_5g_candidate(uint32_t next_candidate,
                                                                  uint32_t candidate_count,
                                                                  char *detail,
                                                                  size_t detail_len)
{
#if AP6256_WIFI_5G_FULL_RECYCLE_AFTER_FAILED_CANDIDATE != 0
    /*
     * HIL with corrected Broadcom event IDs shows the first failed 5 GHz
     * WLC_SET_SSID attempt leaves the firmware session poisoned: the next
     * candidate returns SET_SSID/ABORT almost immediately even though the host
     * still has valid scan evidence. Preserve that host-side candidate list,
     * but do not reuse the firmware session for another BSSID.
     */
    test_uart_printf("[ INFO ] wifi.connect stage: full recycle after failed 5GHz candidate before %lu/%lu\r\n",
    ...
    return ap6256_wifi_runtime_ensure_ready(detail, detail_len);
#else
    test_uart_printf("[ INFO ] wifi.connect stage: bounded drain after failed 5GHz candidate before %lu/%lu\r\n",
                     (unsigned long)next_candidate,
                     (unsigned long)candidate_count);
    ap6256_wifi_runtime_clear_failed_join_for_next_candidate();
    return ap6256_wifi_runtime_recover_radio_before_join("after failed 5GHz candidate",
                                                         detail,
                                                         detail_len);
#endif
}
                                                ...
                                                uint8_t selected_5g)
{
     ...
     *
     * BCM43456/AP6256 transition-mode 5 GHz APs advertise both PSK and SAE
     * with optional MFP. Use the WPA2/CCMP leg when PMF is not required; this
     * mirrors the brcmfmac cfg80211 choice for non-SAE hosts and avoids making
     * firmware pick the SAE side of a transition BSS.
     */
    if ((selected_5g != 0U) &&
        (has_sae != 0U) &&
        (has_psk != 0U) &&
...
#if AP6256_WIFI_5G_FORCE_SAE_AUTH
            return CYW43_AUTH_WPA3_SAE_AES_PSK;
#else
#if AP6256_WIFI_5G_USE_TRANSITION_AUTH
...
static uint32_t ap6256_wifi_runtime_hash_bytes(const uint8_t *data, size_t len)
{
...
static void ap6256_wifi_runtime_capture_iovar_blob_diag(const char *name,
                                                        int32_t *rc_out,
                                                        uint32_t *hash_out,
                                                        uint32_t *first_out,
                                                        uint32_t *second_out)
{
    ...
    *hash_out = ap6256_wifi_runtime_hash_bytes(buf, sizeof(buf));
    *first_out = ap6256_wifi_runtime_get_le32(&buf[0]);
    *second_out = ap6256_wifi_runtime_get_le32(&buf[4]);
}
...
static void ap6256_wifi_runtime_capture_assoc_diag(const char *failure_class,
                                                   ...
                                                   uint8_t keyed_seen)
{
    ap6256_wifi_assoc_diag_t diag;
    uint8_t probed_bssid[6];
    uint8_t diag_ioctl_safe;

    memset(&diag, 0, sizeof(diag));
    diag.valid = 1U;
    (void)snprintf(diag.failure_class, sizeof(diag.failure_class), "%s",
                   (failure_class != NULL) ? failure_class : "UNKNOWN");
    diag.link_status = link_status;
    diag.join_state = cyw43_state.wifi_join_state;
    diag.selected_5g = (selected_5g != 0U) ? 1U : 0U;
    diag.secure = (secure != 0U) ? 1U : 0U;
    diag.assoc_seen = (assoc_seen != 0U) ? 1U : 0U;
    diag.assoc_matches = (assoc_matches != 0U) ? 1U : 0U;
    diag.post_join_event_seen = (post_join_event_seen != 0U) ? 1U : 0U;
    diag.post_join_link_evidence = (post_join_link_evidence != 0U) ? 1U : 0U;
    diag.keyed_seen = (keyed_seen != 0U) ? 1U : 0U;
    diag.target_channel = (uint8_t)((target_channel <= 255U) ? target_channel : 0U);
    diag.target_chanspec = target_chanspec;
    diag.auth_type = auth_type;
    if (target_bssid != NULL) {
        memcpy(diag.target_bssid, target_bssid, sizeof(diag.target_bssid));
    }
    if (assoc_bssid != NULL) {
        memcpy(diag.assoc_bssid, assoc_bssid, sizeof(diag.assoc_bssid));
    }
    diag.join_event_count = ap6256_cyw43_port_join_event_count();
    diag.join_event_type = ap6256_cyw43_port_last_join_event_type();
    diag.join_event_status = ap6256_cyw43_port_last_join_event_status();
    diag.join_event_reason = ap6256_cyw43_port_last_join_event_reason();
    diag.join_event_flags = ap6256_cyw43_port_last_join_event_flags();
    diag.last_rx_class = ap6256_cyw43_port_last_rx_class();
    diag.last_rx_payload_len = ap6256_cyw43_port_last_rx_payload_len();
    diag.last_rx_first_word = ap6256_cyw43_port_last_rx_first_word();

    diag_ioctl_safe =
        ((selected_5g == 0U) ||
         (ap6256_wifi_runtime_5g_can_probe_bssid(secure,
                                                  ap6256_wifi_runtime_join_event_supports_assoc_probe(),
                                                  keyed_seen) != 0U)) ? 1U : 0U;
    /*
     * Keep every post-failure firmware query behind the same association
     * evidence gate. HIL showed a 5GHz no-assoc counters probe reset in HIL
     * while waiting for GET_VAR("counters") immediately after WLC_SET_SSID.
     * That makes counters unavailable for dead 5 GHz joins until the firmware
     * has emitted AUTH/ASSOC/LINK/key evidence.
     */

    memset(probed_bssid, 0, sizeof(probed_bssid));
    diag.get_bssid_rc = -CYW43_EPERM;
    diag.chanspec_rc = -CYW43_EPERM;
    diag.counters_rc = -CYW43_EPERM;
    diag.assoc_info_rc = -CYW43_EPERM;
    if (diag_ioctl_safe != 0U) {
        diag.get_bssid_rc = cyw43_ioctl(&cyw43_state,
                                        AP6256_CYW43_IOCTL_GET_BSSID,
                                        sizeof(probed_bssid),
                                        probed_bssid,
                                        CYW43_ITF_STA);
        diag.get_bssid_valid =
            ((diag.get_bssid_rc == 0) &&
             (ap6256_wifi_runtime_bssid_is_valid(probed_bssid) != 0)) ? 1U : 0U;
        if (diag.get_bssid_valid != 0U) {
            memcpy(diag.assoc_bssid, probed_bssid, sizeof(diag.assoc_bssid));
        }
        diag.chanspec_rc = ap6256_wifi_runtime_get_iovar_u32("chanspec", &diag.chanspec);
        ap6256_wifi_runtime_capture_iovar_blob_diag("assoc_info",
                                                    &diag.assoc_info_rc,
                                                    &diag.assoc_info_hash,
                                                    &diag.assoc_info_first,
                                                    &diag.assoc_info_second);
        ap6256_wifi_runtime_capture_iovar_blob_diag("counters",
                                                    &diag.counters_rc,
                                                    &diag.counters_hash,
                                                    &diag.counters_first,
                                                    &diag.counters_second);
    }

    ap6256_connectivity_set_wifi_assoc_diag(&diag);
    test_uart_printf("[ INFO ] wifi.connect assoc_diag failure_class=%s status=%ld join=0x%08lX target=%02X:%02X:%02X:%02X:%02X:%02X/ch%u/cs%04X auth=0x%08lX\r\n",
                     diag.failure_class,
                     (long)diag.link_status,
                     (unsigned long)diag.join_state,
                     diag.target_bssid[0],
                     diag.target_bssid[1],
                     diag.target_bssid[2],
                     diag.target_bssid[3],
                     ...
                     diag.target_bssid[5],
                     diag.target_channel,
                     diag.target_chanspec,
                     (unsigned long)diag.auth_type);
    test_uart_printf("[ INFO ] wifi.connect assoc_probe assoc=%u/m%u bssid=%ld/%u cs=%ld/0x%04lX ctr=%ld/0x%08lX/%08lX/%08lX ai=%ld/0x%08lX/%08lX/%08lX\r\n",
                     diag.assoc_seen,
                     diag.assoc_matches,
                     (long)diag.get_bssid_rc,
                     diag.get_bssid_valid,
                     (long)diag.chanspec_rc,
                     (unsigned long)diag.chanspec,
                     (long)diag.counters_rc,
                     (unsigned long)diag.counters_hash,
                     (unsigned long)diag.counters_first,
                     (unsigned long)diag.counters_second,
                     (long)diag.assoc_info_rc,
                     (unsigned long)diag.assoc_info_hash,
                     (unsigned long)diag.assoc_info_first,
                     (unsigned long)diag.assoc_info_second);
    test_uart_printf("[ INFO ] wifi.connect assoc_event ev=%lu/%lu/%lu r=%lu f=0x%lX rx=%s/%u first=0x%08lX pje=%u ple=%u keyed=%u\r\n",
                     (unsigned long)diag.join_event_type,
                     (unsigned long)diag.join_event_status,
                     (unsigned long)diag.join_event_reason,
                     (unsigned long)diag.join_event_reason,
                     (unsigned long)diag.join_event_flags,
                     ap6256_wifi_runtime_rx_class_name(diag.last_rx_class),
                     diag.last_rx_payload_len,
                     (unsigned long)diag.last_rx_first_word,
                     diag.post_join_event_seen,
                     diag.post_join_link_evidence,
                     diag.keyed_seen);
}
...
static ap6256_status_t ap6256_wifi_runtime_run_common(const char *ssid,
                                                      ...
                                                      uint8_t mfp,
                                                      uint16_t rsn_cap,
                                                      uint32_t forced_auth_type,
                                                      const uint8_t *bssid,
                                                      uint16_t channel,
                                                      uint16_t chanspec,
                                                      uint8_t use_primary_channel_starter,
                                                      ...
                                                      size_t detail_len)
{
    ...
    if (secure != 0U) {
        ...
        (void)auth_mode;
        if (forced_auth_type != 0U) {
            selected_auth = forced_auth_type;
        } else {
    ...
    ap6256_connectivity_set_wifi_assoc_diag(NULL);
    ...
    ap6256_cyw43_port_reset_join_event_window();
    test_uart_write_str("[ INFO ] wifi.connect stage: join event window reset after scan quiesce\r\n");
    ...
    if ((bssid != NULL) && (channel != 0U) && (channel != AP6256_WIFI_CHANNEL_5G_UNKNOWN)) {
        ...
        if (ap6256_wifi_runtime_channel_is_5g(channel) != 0U) {
             ...
             * but keep the final association command SSID-only. Directed
             * BSSID/chanspec, join/ext_join, and the later WLC_SET_BSSID hint
             * experiments all either reset this board or removed association
             * progress on BCM43456. Older HIL runs showed the firmware can
             * legitimately associate to a same-SSID 5 GHz BSSID that differs
             * from the selected scan entry, then reach DHCP. Keep the selected
             * BSSID as diagnostics only; do not block DHCP on an exact-BSSID
             * match when the outgoing join itself is SSID-only.
             */
#if AP6256_WIFI_5G_DIRECTED_JOIN != 0
            test_uart_write_str("[ INFO ] wifi.connect stage: directed 5GHz BSSID/chanspec join debug path\r\n");
            join_bssid = bssid;
            join_channel = (assoc_target_chanspec != 0U) ?
                CYW43_CHANNEL_FROM_CHANSPEC(assoc_target_chanspec) :
                (uint32_t)channel;
#else
            if (AP6256_WIFI_5G_BSSID_HINT != 0) {
                test_uart_write_str("[ INFO ] wifi.connect stage: cache-backed 5GHz join with WLC_SET_BSSID hint\r\n");
                join_bssid = bssid;
                join_channel = CYW43_CHANNEL_NONE;
            } else if ((use_primary_channel_starter != 0U) ||
                (AP6256_WIFI_5G_PRIMARY_STARTER_FIRST != 0)) {
                test_uart_write_str("[ INFO ] wifi.connect stage: cache-backed ssid-only 5GHz join with primary-channel starter\r\n");
                join_channel = (assoc_target_chanspec != 0U) ?
                    CYW43_CHANNEL_FROM_CHANSPEC(assoc_target_chanspec) :
                    (uint32_t)channel;
                join_bssid = NULL;
            } else {
                test_uart_write_str("[ INFO ] wifi.connect stage: cache-backed ssid-only 5GHz join with band-pref metadata\r\n");
    ...
    if (ap6256_wifi_runtime_channel_is_5g(channel) != 0U) {
        ...
        /*
         * Keep association start free of read-only PHY/caps probes. Several
         * BCM43456 profile runs showed pre-join GET_VAR diagnostics leaving the
         * SDIO control path in an error state immediately before WLC_SET_SSID.
         * Capture Wi-Fi 5/VHT evidence after link/DHCP, when the firmware has
         * a stable association context.
         */
        test_uart_write_str("[ INFO ] wifi.connect stage: defer PHY diag until after link\r\n");
    }
    ...
    rc = cyw43_wifi_join(&cyw43_state,
                         ...
                         join_channel);
    if (ap6256_wifi_runtime_channel_is_5g(channel) != 0U) {
        s_wifi_runtime.poisoned_5g_session = 1U;
    }
    if (rc != 0) {
        (void)snprintf(detail,
                       detail_len,
    ...
    if (!ap6256_wifi_runtime_wait_for_link(AP6256_WIFI_DHCP_TIMEOUT_MS,
        ...
        if (final_status == CYW43_LINK_BADAUTH) {
            reason = "authentication failed";
                   ...
                   (ap6256_wifi_runtime_channel_is_5g(channel) != 0U)) {
            reason = "no AUTH/ASSOC/LINK evidence before guarded 5GHz timeout";
        } else if (final_status == CYW43_LINK_NONET) {
    ...
    if (bssid != NULL) {
        ...
        test_uart_printf("[ INFO ] wifi.connect stage: associated bssid valid=%u %02X:%02X:%02X:%02X:%02X:%02X\r\n",
                         ...
                         assoc_bssid[4],
                         assoc_bssid[5]);
        if ((assoc_bssid_valid != 0U) && (memcmp(assoc_bssid, bssid, sizeof(assoc_bssid)) != 0)) {
            test_uart_printf("[ INFO ] wifi.connect stage: associated bssid differs from selected candidate %02X:%02X:%02X:%02X:%02X:%02X -> %02X:%02X:%02X:%02X:%02X:%02X\r\n",
                             bssid[0],
                             bssid[1],
                             bssid[2],
                             bssid[3],
                                                              ...
                                                              size_t detail_len)
{
    ...
    if (ap6256_wifi_runtime_channel_is_5g(selected_copy.channel) != 0U) {
        if (!ap6256_wifi_runtime_recycle_after_5g_scan_before_join(candidate_index,
                                                                   candidate_count,
                                                                   detail,
                                                                   detail_len)) {
            ap6256_connectivity_set_wifi_note(detail);
        ...
        detail[0] = '\0';
    } else if (!ap6256_wifi_runtime_recover_radio_before_join("after scan", detail, detail_len)) {
        ap6256_connectivity_set_wifi_note(detail);
        return AP6256_STATUS_IO_ERROR;
    }
    ...
    if ((st != AP6256_STATUS_OK) &&
        (ap6256_wifi_runtime_should_retry_primary_channel_starter(&selected_copy,
                                                                  st,
                                                                  candidate_index,
                                                                  detail) != 0U)) {
        test_uart_printf("[ INFO ] wifi.connect stage: retry candidate %u/%u with 5GHz primary-channel starter ev=%lu/%lu/%lu\r\n",
                         candidate_index,
                         candidate_count,
                         (unsigned long)ap6256_cyw43_port_last_join_event_type(),
                         (unsigned long)ap6256_cyw43_port_last_join_event_status(),
        ...
        ap6256_wifi_runtime_clear_failed_join_for_next_candidate();
        detail[0] = '\0';
        if (!ap6256_wifi_runtime_recover_radio_before_join("before primary-channel retry",
                                                           detail,
                                                           detail_len)) {
            ap6256_connectivity_set_wifi_note(detail);
            return AP6256_STATUS_IO_ERROR;
...
void ap6256_wifi_runtime_suspend(void)
{
    ...
    if (s_wifi_runtime.poisoned_5g_session != 0U) {
        test_uart_write_str("[ INFO ] wifi.connect stage: hard suspend poisoned 5GHz session\r\n");
        cyw43_poll = NULL;
        ap6256_power_down();
        ap6256_cyw43_port_deinit();
        ap6256_wifi_runtime_reset_driver_state();
        ...
        ap6256_connectivity_set_wifi_ip("", "", "", 0U);
        ap6256_connectivity_set_wifi_runtime(1U, 0U, s_wifi_runtime.last_scan_count, s_wifi_runtime.last_rssi);
        ap6256_connectivity_set_wifi_note("BCM43456 Wi-Fi runtime hard-suspended after failed 5GHz join.");
        return;
    }
                                                ...
                                                size_t detail_len)
{
    ...
    if (preferred_channel > 14U) {
        test_uart_printf("[ INFO ] wifi.connect stage: profile directed 5GHz scan ssid=%s requested_ch=%u\r\n",
                         ssid,
                         (unsigned)preferred_channel);
        selected = ap6256_wifi_runtime_resolve_directed_5g_selection_channel(ssid,
                                                                             preferred_channel,
                                                                             detail,
                                                                             detail_len);
        if (selected != NULL) {
    ...
    if (join_candidate_count != 0U) {
        ...
        for (uint32_t i = 0U; i < join_candidate_count; ++i) {
            ...
            if ((i != 0U) && (ap6256_wifi_runtime_channel_is_5g(attempt.channel) != 0U)) {
                /*
                 * Keep the original scan evidence for all 5 GHz candidates.
                 * Restarting CYW43 and re-running a directed scan between
                 * candidates was slow and fragile: later candidates could
                 * spend the full scan timeout in "cache prime" without ever
                 * attempting association. brcmfmac keeps the BSS table and
                 * walks candidates from that evidence; do the same here.
                 */
                test_uart_printf("[ INFO ] wifi.connect stage: reuse scan evidence for candidate %lu/%lu ch=%u\r\n",
                                 (unsigned long)(i + 1U),
...
#if AP6256_WIFI_5G_STOP_AFTER_POISONED_SESSION != 0
            if ((ap6256_wifi_runtime_channel_is_5g(attempt.channel) != 0U) &&
                (s_wifi_runtime.poisoned_5g_session != 0U)) {
                test_uart_printf("[ INFO ] wifi.connect stage: stop 5GHz candidate retry after poisoned session candidate %lu/%lu failure_class=CMD_ACK_NO_AUTH/SESSION_POISONED\r\n",
                                 (unsigned long)(i + 1U),
                                 (unsigned long)join_candidate_count);
                break;
            }
            ...
            if ((i + 1U) < join_candidate_count) {
                if (ap6256_wifi_runtime_channel_is_5g(attempt.channel) != 0U) {
                    detail[0] = '\0';
                    if (!ap6256_wifi_runtime_recycle_after_failed_5g_candidate(i + 2U,
                                                                               join_candidate_count,
                                                                               detail,
                                                                               detail_len)) {
                        if (detail[0] == '\0') {
                            (void)snprintf(detail,
                                           detail_len,
                                           "Failed to recycle Wi-Fi firmware before candidate %lu/%lu.",
                                           (unsigned long)(i + 2U),
                                           (unsigned long)join_candidate_count);
                        }
                        ap6256_connectivity_set_wifi_note(detail);
